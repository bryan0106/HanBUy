...
interface PasabuyModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (request: PasabuyRequest) => Promise<void>;
}

export interface PasabuyRequest {
  product_url?: string;
  product_name?: string;
  comment?: string;
}

export function PasabuyModal({
  isOpen,
  onClose,
  onSubmit,
}: PasabuyModalProps) {
  const [formData, setFormData] = useState<PasabuyRequest>({
    product_url: "",
    product_name: "",
    comment: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.product_url?.trim() && !(formData.comment || "").trim()) {
      setError("Please provide either a product URL or describe what product you're looking for");
      return;
    }
    ...
    try {
      ...
      setError(err.message || "Failed to submit pasabuy request. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex items-center justify-between border-b border-border p-4 sm:p-6 shrink-0">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground sm:text-sm">
              Can't find what you're looking for? We'll find and buy it for you!
            </p>
          </div>
          <button
            onClick={handleClose}
          ...
          <div className="space-y-3 sm:space-y-4 overflow-y-auto flex-1">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Paste the link to the product you want us to buy for you
              </p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-border bg-background px-2.5 py-1.5 text-sm focus:border-soft-blue-600 focus:outline-none focus:ring-2 focus:ring-soft-blue-600/20 sm:px-3 sm:py-2"
                placeholder="e.g., BTS Album, Korean Skincare Set"
                disabled={submitting}
              />
              <p className="mt-1 text-xs text-muted-foreground">
                Help us identify what product you're looking for
              </p>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe the product you want us to find and buy for you. Include details like brand, size, color, quantity, etc..."
                disabled={submitting}
              />
              <p className="mt-1 text-xs text-muted-foreground">
                {formData.product_url?.trim() 
                  ? "Add any additional notes or special requirements"
                  : "Describe the product you want. We'll search for it and buy it for you!"}
              </p>
            </div>
          </div>
